...
typedef long long T; // type of the elements of the tree (may be ll if needed)
T seg_tree[MAXN*4];
const T NEUTRAL = 0; //neutral element for the operation (sum = 0, min = +inf, max = -inf). Zero is also the neutral element in XOR operation.
T calculate(T a, T b) {
    return a ^ b;
}

//"node" is the index in the array, while "left" "Nó" é o índice do array, enquanto "esquerda" e "direita" são o fim dos segmentos específicos
...
void update(int node, int left, int right, int index, int value) {
    if (left == right) { //we are in the xth leaf
        ...
        if (index <= middle) { //we need to update the left son
...
int main(){
    ...
    for(long long unsigned int  k; k < queries.size(); k ++) cout << queries[k] << endl;
}